...
class TypeReference {
	...
	get genericParameters() {
		...
		if(!this._genericParameters) {
			...
			if(genericParameters.length)
				log.debug(`inferring genericParameters for ${this.namespacedName}: `, genericParameters);
	...
	mapGenericArguments(other, genericArgumentMap) {
		if(this.namespacedName !== other.namespacedName)
			return false;
		let thisGenericArguments = this.genericArguments,
			otherGenericArguments = other.genericArguments;
		assert.equal(thisGenericArguments.length, otherGenericArguments.length);
		if(!genericArgumentMap)
			genericArgumentMap = new Map; // this -> other
		for(let i = 0; i < thisGenericArguments.length; i++) {
			let a = thisGenericArguments[i],
				b = otherGenericArguments[i];
			if(isGenericParameter(a)) {
				...
				log.debug(`${other} is more generic than ${this}`);
				return false;
	...
	toSimpleType(simpler=false) {
		...
		if(this.genericArguments.length)
			str += "<" + this.genericArguments.map(a => {
				if(isGenericParameter(a))
					return a;
				return a.toSimpleType(simpler);
			}).join(", ") + ">";
		return str;
	}
