...
const { isPlainObject, cloneJsonObject } = require("../utils");
...
function wrapSyncElementField(field) {
	class SyncElement extends mixinISyncMember(mixinMember(mixinIWorldElement(wrapBaseField(field)))) {
		...
		get value() {
			return new (this.field.valueType)(cloneJsonObject(this.obj.Data));
		}
	...
	class WorkerBag extends mixinISyncCollection(wrapSyncElementField(inner)) {
		...
		constructor(obj) {
			...
			this.items = (this.obj.Data || []).map(obj => {
				...
				return this._createWorker(workerType, obj.Data);
			});
		}

		_createWorker(workerType, obj) {
			return new workerType(obj);
		}

		attachWorker(workerType, obj) {
			workerType = wrapType(workerType);
			let worker = this._createWorker(workerType, obj);
			this.addItem(worker);
			return worker;
		}
