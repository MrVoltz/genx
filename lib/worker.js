...
const { type, registerTypeWrapperResolver, resolveTypeWrapper, isNeosPrimitive } = require("./neos-types");
...
class SyncValue extends mixinMember(Object) {
	constructor(type, obj) {
		super();
		this.type = type;
		this.obj = obj;
		// TODO:
		assert(isNeosPrimitive(this.type));
	}
...
class SyncMember extends mixinMember(mixinWorldElement(Object)) {
	...
	get valueType() {
		let syncType = type("Sync<T>", ["T"]).isAssignableFrom(this.type);
		return syncType && syncType.get("T");
	}

	get value() {
		return new SyncValue(this.valueType, cloneJsonObj(this.obj.Data));
	}
	...
	assign(value) {
		if(value instanceof SyncValue) {
			if(type("RefID").equals(this.valueType))
				console.log(`assigning SyncValue to ${this.type}, this may be not intentional`);
			assert(this.valueType.equals(value.type))
			this.assignValueObj(value.toJSON());
		} else if(value instanceof Worker || value instanceof SyncMember) {
			let syncRefType = type("SyncRef<T>", ["T"]).isAssignableFrom(this.type);
			assert(syncRefType);
			assert(syncRefType.get("T").isAssignableFrom(value.type));
			this.assignValueObj(value.refId);
		} else
			throw new Error(`cannot assign ${value} to ${this.type}`);
	}
...
function createConstant(valueType, obj) {
	valueType = type(valueType);
	return new SyncValue(valueType, obj);
}

function createAutoConstant(obj) {
	// TODO
}

module.exports = { Worker, createWorker, mixinWorldElement, mixinMember, SyncMethod, SyncMember, createSyncMember, SyncValue, createConstant };
